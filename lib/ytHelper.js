...
/**
 * Robust YouTube search using Innertube API with fallback
 * @param {string} query 
 * @returns {Promise<{videos: Array<{type: string, videoId: string, url: string, title: string, thumbnail: string, image: string, timestamp: string, seconds: number, views: string, ago: string}>}>}
 */
export async function searchYouTube(query) {
    if (!query) return { videos: [] }

    // Phase 1: YouTube Innertube API (Instant, Reliable, No HTML breakage)
    try {
    ...
    // Phase 2: yt-search library fallback
    try {
