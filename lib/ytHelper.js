...
export async function downloadYT(url, type = 'video') {
    ...
    // Phase 1: Cobalt API - Primary resolver (prevents VPS IP bans)
    for (const api of COBALT_APIS) {
        try {
            console.log(`[YouTube Downloader] Phase 1: Resolving via Cobalt API (${api})...`)
            ...
            if (data && (data.status === 'tunnel' || data.status === 'redirect') && data.url) {
                console.log(`[YouTube Downloader] Cobalt successfully resolved URL: ${data.filename || 'media'}`)
            ...
            console.warn(`[YouTube Downloader] Cobalt API (${api}) failed: ${err.message}.`)
        }
    }
