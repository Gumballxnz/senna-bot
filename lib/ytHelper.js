...
export async function downloadYT(url, type = 'audio') {
    ...
    try {
        ...
        // [NOVO] MOTOR PRINCIPAL: Vreden API (Alta estabilidade para vídeos longos)
        try {
            let vr = await fetch(`https://api.vreden.web.id/api/ytvideo?url=${encodeURIComponent(url)}`).then(v=>v.json()).catch(()=>null)
            dl_url = vr?.result?.links?.['720p']?.download || vr?.result?.links?.['480p']?.download || vr?.result?.url
            title = vr?.result?.title || title
        } catch(e) {}
        ...
        if (!dl_url) {
            // Fallback 2: Lolibot API (Resiliente a arquivos > 300MB)
        ...
        if (!dl_url) {
            // Fallback 3: Ryzendesu (MODO RESULT)
        ...
        // Download com Stream Direto e sem Headers de Browser (evita throttling de algumas APIs)
