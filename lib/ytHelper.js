...
export async function downloadYT(url, type = 'audio') {
    ...
    try {
        ...
        // Tentar buscar qualidade HD via API Dylux/FG
        let res = type === 'audio' ? await fg.yta(url).catch(() => null) : await fg.ytv(url).catch(() => null)
        
        // Se for vídeo, tentar forçar 720p no objeto de resultado (se a lib suportar)
        dl_url = res?.dl_url || res?.result || res?.data?.dl_url
        title = res?.title || title

        if (!dl_url) {
            // Fallback 1: API de Alta Qualidade (Decifrando links HD)
            let api720 = await fetch(`https://api.vreden.web.id/api/ytvideo?url=${encodeURIComponent(url)}`).then(v=>v.json()).catch(()=>null)
            dl_url = api720?.result?.links?.['720p']?.download || api720?.result?.links?.['480p']?.download || api720?.result?.url
        }

        if (!dl_url) {
            // Fallback 2: Ryzendesu (Geralmente 720p/480p)
        ...
        if (!dl_url) throw new Error('O YouTube bloqueou a extração HD para este vídeo gigante. Tente reduzir a duração.')
