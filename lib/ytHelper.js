...
export async function downloadYT(url, type = 'video') {
    ...
    try {
        console.log(`[YouTube Downloader] Phase 1: Resolving URL in parallel via Cobalt APIs... (${targetUrl})`)
    ...
    try {
        ...
        if (type === 'audio') {
            cmd = `yt-dlp -f "ba[ext=m4a]/ba/b" --extract-audio --audio-format mp3 --audio-quality 128k --no-playlist --no-warnings -o "${filePath}" "${targetUrl}"`
        } else {
            cmd = `yt-dlp -f "bv*[height<=720][ext=mp4]+ba[ext=m4a]/bv*[height<=720]+ba/b[height<=720]/b" --merge-output-format mp4 --no-playlist --no-warnings -o "${filePath}" "${targetUrl}"`
        }
...
export async function getYTInfo(url) {
    ...
    try {
        const { stdout } = await execAsync(
            `yt-dlp --no-warnings --print "%(title)s" "${targetUrl}"`,
            { timeout: 15000 }
        )
