...
export async function downloadYT(url, type = 'audio') {
    ...
    try {
        ...
        if (useFallback) {
            console.log(`Fallback ativado para: ${url}`)
            ...
            if (!dl_url) throw new Error('Falha total: YouTube bloqueou todos os métodos de acesso.')

            let dl = await fetch(dl_url)
            if (!dl.ok) throw new Error(`Falha no download da API: ${dl.statusText}`)
            await pipeline(dl.body, fs.createWriteStream(filePath))
            
            if (type === 'audio' && fs.existsSync(filePath)) {
                const finalPath = filePath.replace('.mp3', '_fixed.mp3')
                try {
                    await execAsync(`ffmpeg -i "${filePath}" -b:a 128k -map a -f mp3 "${finalPath}" -y`, { timeout: 60000 })
                    if (fs.existsSync(finalPath)) {
                        if (fs.existsSync(filePath)) fs.unlinkSync(filePath)
                        fs.renameSync(finalPath, filePath)
                    }
                } catch (ffErr) {
                    console.error('Erro no FFmpeg fallback:', ffErr.message)
                }
            }
        }
