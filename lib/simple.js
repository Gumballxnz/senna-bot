    ...
    let sock = Object.defineProperties(conn, {
        ...
        getFile: {
            ...
            async value(PATH, saveToFile = false) {
                let res, filename
                const data = Buffer.isBuffer(PATH) ? PATH : PATH instanceof ArrayBuffer ? PATH.toBuffer() : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split`,`[1], 'base64') : /^https?:\/\//.test(PATH) ? await (res = await fetch(PATH, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36' } })).buffer() : fs.existsSync(PATH) ? (filename = PATH, fs.readFileSync(PATH)) : typeof PATH === 'string' ? PATH : Buffer.alloc(0)
...
if (mtype === 'audio') {
  ...
  if (mtype === 'video' && pathFile && fs.existsSync(pathFile)) {
    try {
      ...
      if (codec) {
        const readyPath = path.join(__dirname, '../tmp/ready_' + Date.now() + '.mp4')
