    ...
    let sock = Object.defineProperties(conn, {
        ...
        getGroupMetadata: {
            async value(jid) {
                ...
                try {
                    const data = await this.groupMetadata(jid)
                    this._groupMetadataCache.set(jid, { data, ts: now })
                    return data
                } catch (e) {
        ...
        getFile: {
            ...
            async value(PATH, saveToFile = false) {
                ...
                if (typeof PATH === 'string') {
                    if (fs.existsSync(PATH)) localFile = PATH
                    else if (fs.existsSync(join(process.cwd(), PATH))) localFile = join(process.cwd(), PATH)
                    else if (fs.existsSync(join(__dirname, '../', PATH))) localFile = join(__dirname, '../', PATH)
                    else if (fs.existsSync(join(__dirname, '../src', path.basename(PATH)))) localFile = join(__dirname, '../src', path.basename(PATH))
                }
                const data = Buffer.isBuffer(PATH) ? PATH : PATH instanceof ArrayBuffer ? PATH.toBuffer() : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split`,`[1], 'base64') : /^https?:\/\//.test(PATH) ? await (res = await fetch(PATH, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36' } })).buffer() : localFile ? (filename = localFile, fs.readFileSync(localFile)) : Buffer.alloc(0)
                if (!Buffer.isBuffer(data) || data.length === 0) throw new TypeError(`Arquivo não encontrado ou buffer inválido: ${PATH}`)
                const type = await fileTypeFromBuffer(data) || {
                    mime: 'application/octet-stream',
                    ext: '.bin'
                }
                if (data && saveToFile && !filename) (filename = join(__dirname, '../tmp/' + new Date * 1 + '.' + type.ext), await fs.promises.writeFile(filename, data))
                return {
                    res,
                    filename,
                    ...type,
...
async value(jid, path, filename = '', caption = '', quoted = null, ptt = false, options = {}) {
  ...
  if (options.asDocument) {
    ...
    mtype = 'sticker'
  } else if (options.asImage || /image\/(jpeg|png|jpg|gif|bmp)/i.test(mimetype)) {
    mtype = 'image'
  } else if (/webp/i.test(mimetype)) {
    if (caption || options.caption || (filename && !filename.endsWith('.webp')) || options.asImage) {
      mtype = 'image'
    } else {
      mtype = 'sticker'
    }
...
if (mtype === 'audio') {
  ...
  if (mtype === 'image') {
    if (/webp/i.test(mimetype) || (Buffer.isBuffer(file) && file.slice(0, 4).toString() === 'RIFF')) {
      try {
        const sharp = (await import('sharp')).default
        file = await sharp(file).jpeg({ quality: 92 }).toBuffer()
        mimetype = 'image/jpeg'
      } catch (e) {
        console.error('Erro ao converter webp para jpeg no sendFile:', e.message)
      }
    }
  }

  if (mtype === 'video' && pathFile && fs.existsSync(pathFile)) {
    try {
      const { stdout } = await execAsync(`ffprobe -v error -select_streams v:0 -show_entries stream=codec_name -of default=noprint_wrappers=1:nokey=1 "${pathFile}"`).catch(() => ({ stdout: '' }))
      const codec = stdout ? stdout.trim().toLowerCase() : ''
      if (codec) {
        const readyPath = join(__dirname, '../tmp/ready_' + Date.now() + '.mp4')
        const cmd = (codec === 'h264' || codec === 'h264(high)')
          ? `ffmpeg -y -i "${pathFile}" -c copy -movflags +faststart "${readyPath}"`
          : `ffmpeg -y -i "${pathFile}" -c:v libx264 -preset ultrafast -crf 26 -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -c:a aac -b:a 128k -movflags +faststart "${readyPath}"`
        await execAsync(cmd, { timeout: 60000 })
        if (fs.existsSync(readyPath)) {
          file = await fs.promises.readFile(readyPath)
          mimetype = 'video/mp4'
          try { fs.unlinkSync(readyPath) } catch (_) {}
        }
      }
    } catch (vErr) {
      console.error('Erro ao sanitizar video no sendFile:', vErr.message)
    }

    if (file && file.length > 60 * 1024 * 1024) {
      mtype = 'document'
      mimetype = 'video/mp4'
    }
  }
  ...
  try {
    ...
    if (type && typeof type.deleteFile === 'function') {
      try { await type.deleteFile() } catch (_) {}
    }
  }
},
enumerable: true
...
export function serialize() {
    const MediaType = ['imageMessage', 'videoMessage', 'audioMessage', 'stickerMessage', 'documentMessage', 'viewOnceMessage', 'viewOnceMessageV2', 'viewOnceMessageV2Extension']
    ...
    enumerable: true
        ...
        mtype: {
            get() {
                ...
                const Message = extractMessageContent(this.message) || this.message
        ...
        msg: {
            get() {
                ...
                const Message = extractMessageContent(this.message) || this.message
                return Message[this.mtype] || Message[Object.keys(Message)[0]] || null
            }
        },
        mediaMessage: {
            get() {
        ...
        quoted: {
            get() {
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
                    ...
                    mediaMessage: {
                        get() {
                            let Message = ((q.url || q.directPath) ? { ...quoted } : extractMessageContent(quoted)) || null
                            if (!Message) return null
                            let mtype = Object.keys(Message)[0]
                            if (['viewOnceMessage', 'viewOnceMessageV2', 'viewOnceMessageV2Extension'].includes(mtype)) {
                                Message = extractMessageContent(Message[mtype]?.message || Message[mtype])
                                if (!Message) return null
        ...
        text: {
            get() {
                if (typeof this._text === 'string' && this._text.length > 0) return this._text
                const msg = this.msg
...
download: {
    ...
    async value(text, chatId, options = {}) {
        ...
        const formattedText = typeof text === 'string' ? text : format(text)
