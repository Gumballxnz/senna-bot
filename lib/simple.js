    ...
    enumerable: true
        ...
        msg: {
            get() {
                ...
                let m = this.message[this.mtype]
                if (['viewOnceMessage', 'viewOnceMessageV2', 'viewOnceMessageV2Extension'].includes(this.mtype)) {
                    m = m?.message?.imageMessage || m?.message?.videoMessage || m?.message?.audioMessage || m?.message || m
                }
                return m
            }
        },
        mediaMessage: {
            get() {
                if (!this.message) return null
                let Message = ((this.msg?.url || this.msg?.directPath) ? { ...this.message } : extractMessageContent(this.message)) || null
