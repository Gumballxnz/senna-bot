  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let ff = spawn('ffmpeg', [
        '-y',
        '-i', inp,
        '-vf', 'scale=512:512:force_original_aspect_ratio=increase,crop=512:512,format=rgba,setsar=1',
        '-f', 'png',
        '-'
      ])
      ff.on('error', reject)
      ...
      let bufs = []
      const useGm = support.gm
      const [_spawnprocess, ..._spawnargs] = [...(useGm ? ['gm'] : []), 'convert', 'png:-', 'webp:-']
...
function sticker6(img, url) {
  return new Promise(async (resolve, reject) => {
    ...
    Fffmpeg
      ...
      .addOutputOptions([
        `-vcodec`, `libwebp`, `-vf`,
        `scale=512:512:force_original_aspect_ratio=increase,crop=512:512,fps=15,format=rgba,setsar=1`
      ])
      .toFormat('webp')
      .save(out)
  })
