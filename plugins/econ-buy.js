...
let handler = async (m, { conn, text, usedPrefix, command, args }) => {
  if (!args[1]) throw `📌 Exemplo: *${usedPrefix + command}* <ID> <cantidad>\nExemplo: *${usedPrefix + command}* _01_ 5\n\n*${usedPrefix}shop* Para ver todos los *Items*`;
...
└──────────────`, null, fwc);
      ...
      m.reply(`❎ Você não tem suficientes Coins para comprar *${count}* 💎`, null, fwc);
    }
  } else if (option === '02') {
    let count = 0;
    let unit = '';
